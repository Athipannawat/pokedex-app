  ...
  const callData =async (name:string) => {
    ...
    if(response.status === 200){
      ...
      setPokemon({ 
        ...
        response.data.sprites.other['official-artwork'].front_default,}, loading: false , error: null })   
    }else{
  ...
  return (
    <div className='w-[90%] m-[auto] max-w-[1100px]'>
      ...
      <div className='w-[90%] max-w-[600px] m-[auto]'>
        <Link to={"/"} className='bg-[#4CAFED] px-[16px] py-[4px] rounded-[16px] font-semibold'>Back</Link>
        {pokemon.data && 
        (<div className="rounded-[20px] overflow-hidden shadow p-[16px] m-[auto]">
          <div className="bg-center aspect-square w-full bg-cover rounded-[20px] relative" >
            <img className="absolute h-[auto] max-h-[400px] aspect-square translate-x-[-50%] translate-y-[-50%] top-[50%] left-[50%]" src='/images/pokemon_bg.png' alt="" />
          ...
          <div className="pt-5 bg-[#253641] rounded-[20px] p-[16px] my-[20px]">
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 gap-x-[20px] gap-y-[30px]'>
              <div>
                <div className='flex gap-x-[10px]' >
                  <div className='text-[#4CAFED] font-semibold' >Height</div>
                  <div className='text-white' >{(pokemon.data.height/10).toFixed(2)} m.</div>
                </div>
                <div className='flex gap-x-[10px]' >
                  <div className='text-[#4CAFED] font-semibold' >weight</div>
            ...
            <div>
              ...
              {pokemon.data.abilities.map((item)=>{
                return <div className={`bg-[#4CAFED] px-[14px] capitalize py-1 rounded-[16px]`}>{item.ability.name}</div>
              })}
              </div>
            </div>
              ...
              {pokemon.data.stats.map((item)=>{
                return (
                  <div className='flex gap-x-[10px] justify-between' >
                    <div className='text-[#4CAFED] font-semibold capitalize'>{item.stat.name}</div>
